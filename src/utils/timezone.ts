...
export class TimezoneService {
  ...
  async detectTimezoneFromLocation(location: string): Promise<string> {
    ...
    logger.warn(`Could not detect timezone for location: ${location}, using default`);
  ...
  convertTimeToTimezone(
    ...
    try {
      ...
      const dateInSourceTz = new Date(`${dateStr}T${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:00`);
