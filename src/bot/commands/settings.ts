...
export class SettingsCommand {
  async getReminderSettings(phoneNumber: string): Promise<string> {
    try {
      ...
      for (const setting of settings) {
        ...
        message += `${this.getReminderTypeEmoji(
          setting.reminder_type
        )} ${this.formatReminderType(setting.reminder_type)}: ${status}\n`;
  ...
  async setReminderSetting(
    ...
    try {
      ...
      return `✅ ${this.formatReminderType(
        reminderType
      )} reminder ${status}.\nTime: ${offsetText}`;
    } catch (error) {
  ...
  async parseTimeOffset(input: string): Promise<number | null> {
    ...
    const beforeMatch = normalized.match(
      /(\d+)\s*(?:min|minute|minutes)?\s*before/
    );
    if (beforeMatch) {
    ...
    const afterMatch = normalized.match(
      /(\d+)\s*(?:min|minute|minutes)?\s*after/
    );
    if (afterMatch) {
