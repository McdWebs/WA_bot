...
export class RegistrationCommand {
  async handleNewUser(phoneNumber: string, messageBody: string): Promise<string> {
    try {
  ...
  private async handleRegistrationStep(user: User, messageBody: string): Promise<string> {
    ...
    if (!user.location) {
      ...
      const timezone = await timezoneService.detectTimezoneFromLocation(location);
